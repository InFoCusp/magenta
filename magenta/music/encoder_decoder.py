...
class PitchDifferenceEncoderDecoder(object):
  ...
  def encode(self, events):
    ...
    for i in range(1, len(events) - 1):
      ...
      inputs.append(self.events_to_input(prev_event, curr_event, last_note))
      if curr_event not in {-1, -2}:
        last_note = curr_event
      labels.append(self.events_to_label(curr_event, next_event, last_note))
    # import pdb
    # pdb.set_trace()
    return sequence_example_lib.make_sequence_example(inputs, labels)
  ...
  def get_inputs_batch(self, event_sequences, full_length=False):
    ...
    for events in event_sequences:
      ...
      if full_length:
        ...
        # Twisha - Super Doubtful
        index = len(events) - 1
        last_note = events[index]
        while index>0 and last_note in {-1,-2}:
          index-=1
          last_note = events[index]

        inputs.append(self.events_to_input(events[len(events) - 2], events[len(events) - 1], last_note))
      inputs_batch.append(inputs)
    return inputs_batch
  ...
  def extend_event_sequences(self, event_sequences, softmax):
    ...
    for i in range(len(event_sequences)):
      ...
      if event_diff < (self._min_note - self._max_note):
        ...
        while ind>0 and last_event in {-1, -2}:
          ind -= 1
          last_event = event_sequences[i][ind]
        next_note = last_event + event_diff
  ...
  def evaluate_log_likelihood(self, event_sequences, softmax):
    ...
    for i in range(len(event_sequences)):
      ...
      # import pdb
      # pdb.set_trace()
      last_note = event_sequences[i][start_pos]
      for softmax_pos, position in enumerate(range(start_pos+1, end_pos)):
        index = self.events_to_label(event_sequences[i][position-1], event_sequences[i][position], last_note)
        if event_sequences[i][position] not in {-1,-2}:
          last_note = event_sequences[i][position]
        if isinstance(index, numbers.Number):
          loglik += np.log(softmax[i][softmax_pos][index])
        else:
          for sub_softmax_i in range(len(index)):
...
class PitchDifferenceOneHotEventSequenceEncoderDecoder(PitchDifferenceEncoderDecoder):
  ...
  def default_event_label(self):
    return self._one_hot_encoding.encode_event(
        self._one_hot_encoding.default_event, self._one_hot_encoding.default_event,
        self._one_hot_encoding.default_event)
...
# Twisha - Added this class equivalent to OneHotEncoing
class PitchDifferenceOneHotEncodingBasic(object):
  """An interface for specifying a one-hot encoding of individual events."""
  __metaclass__ = abc.ABCMeta
...
# Twisha - Added this to change the encode class. Equivalent to EventSequenceEncoderDecoder
class PitchDifferenceEncoderDecoderBasic(object):
...
# Twisha - Added this one hot encoder class. Equivalent to OneHotIndexEventSequenceEncoderDecoder
class PitchDifferenceOneHotEventSequenceEncoderDecoderBasic(PitchDifferenceEncoderDecoderBasic):
  """An EventSequenceEncoderDecoder that produces a one-hot encoding."""
...
# Twisha - Adding a new class for supporting two encoder decoders encode function - one for input other for output
class MelodyPitchDifferenceEncoderDecoder(object):
    __metaclass__ = abc.ABCMeta
    ...
    def __init__(self, input_one_hot_encoding, output_one_hot_encoding):
      ...
      self._min_note = output_one_hot_encoding._min_note
      self._max_note = output_one_hot_encoding._max_note
...
class MelodyPitchDifferenceOneHotEventSequenceEncoderDecoder(MelodyPitchDifferenceEncoderDecoder):
  """An EventSequenceEncoderDecoder that produces a one-hot encoding."""
  ...
  def events_to_input(self, curr_event):
    ...
    input_ = [0.0] * self.input_size
    # input_[self._one_hot_encoding.encode_event(events[position])] = 1.0
    index = self.input_one_hot_encoding.encode_event(curr_event)
    input_[index] = 1.0
    # import pdb
    # pdb.set_trace()
    return input_
